...
总体架构定义系统模块和职责边界。游戏包规范定义只读目录契约。异步内容加载文档定义 Loader 与 Repository。Session Coordinator 文档定义异步内容准备、统一暂停和 Runtime 原子提交之间的协调边界。Runtime Core 文档定义权威游戏状态、同步命令、Tick、移动、碰撞、Portal、Runtime Effect 和地图切换事务。Client Scoped State Tree 协议定义 Runtime Server 与 Web Client 之间的通用客户端状态格式。
...
9. Runtime Core 不执行文件或网络 I/O，并以同步、确定性的事务维护权威游戏状态。
10. Runtime Core 不依赖 Client State、Scope Tree、Runtime Service 或 DOM。
11. 手动暂停、过场暂停和加载暂停共享统一暂停语义。
12. Runtime 内部状态不得直接序列化给客户端。
13. Client State 使用 Scope、Roots 和通用节点树，不固定地图、人物、菜单等业务 DTO。
14. 每个 Client Node 通过稳定 Key、注册 Tag、JSON Data 和 Children 映射到一个 DOM Element。
15. 图片资源由 Web Client 通过 Runtime Service 按资源 Key 请求。
16. LoomRealm 不提供游戏内容编辑器或项目创作接口。
17. 存档系统暂缓，不进入第一阶段闭环。
