...
> 关系：clarifies / extends [ADR 0002：平台 Transport Binding](./0002-platform-transport-profiles.md)
...
但 Broker 不拥有 generation，也不能从 endpoint/Port 推导 authority。
...
Desktop/PWA composition 负责将同一业务 definition 运行在不同平台。
...
- Main 不直接等同 Node launcher/WebSocket；
...
- 多 Renderer / remote Runtime 等新拓扑要求扩展 Platform coordination model。

仅仅增加一个新 Transport 或换一种 Process/Worker API，不足以修改 application architecture。
