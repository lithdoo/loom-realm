...
冻结前复核发现，若不提前关闭以下问题，实施阶段仍会被迫做架构决策：
...
本 ADR 关闭这些决策，并规定后续实现只能在发现 correctness/security contradiction 时 reopen，不得因编码便利重新设计边界。

---

## 决策 1：M7 冻结一个最窄 `RendererControlBinding`
...
## 决策 2：opaque material capability 收敛为 `OpaqueMaterialGenerator`
...
因此 M7 将 material source 重命名/泛化为：
...
Main-facing Platform view使用同一窄 capability取得**独立 fresh value**：
...
## 决策 3：M7 `MainPlatform` capability view 精确扩展
...
这是 Main 当前真实需要的 role-local structural view，不是 universal Platform interface。

Main 的 Renderer accept loop 是 Session 内部 orchestration：

```text
Session live
→ generate/register fresh rendererControlToken
→ rendererControl.acquire(token, session-attempt signal)
→ candidate carrier
→ renderer-control Main peer
→ typed renderer.hello
→ Main atomic hello acceptance
→ current/replacement
→ arm next fresh acquire when bounded attempt state permits
```

Renderer absence不得阻塞 Runtime/Frame Session 业务执行；pending acquire 是独立 background capability wait。

---

## 决策 4：hello representability preflight 必须发生在 current switch 之前

`@loomrealm/renderer-control` MUST 提供一个 side-effect-free exact outbound preparation/preflight capability，供 Main 在 serialized authority lane 内确认 `renderer.hello` Success 对应的**完整实际 JSON text application unit**可合法表示。

精确函数名不属于架构契约，但语义冻结：

```text
input  = protocolVersion 1 + request id 1 + Snapshot R
...
## 决策 5：revision / Snapshot source 冻结
...
## 决策 6：replacement 是 active revocation，但不虚构 send cancellation
...
但 Foundation `MessageCarrier.send()` 只承诺 local acceptance，不承诺可取消已开始的 send。因此 Core 能冻结的保证是：
...
## 决策 7：Renderer hello handoff 冻结
...
因此 later state surface必须 lazy/explicit-start 或等价地保证 initial install 先发生；不允许 callback 在 role 安装 R 前抢先暴露 R+1。

Renderer role不建立第二套 revision/session validator。

---

## 决策 8：Session terminal 是 Renderer authority terminal boundary
...
## 决策 9：Runtime lifecycle projection 不再留 reopen 条件
...
## 决策 10：representation failure 只终止 Renderer Control
...
## 决策 11：M7 closure 与后续 physical qualification 分离
...
@loomrealm/main projection/revision/accept-loop/currentness
...
3. 冻结 M7 `RendererControlBinding` exact semantics；
4. 对 `phase-1-delivery-plan.md` 原两行 M7 summary给出更精确的 closure定义：M7关闭 logical Binding + MemoryCarrier semantic vertical，不要求 Hostra/PWA physical Renderer transport；
5. 对 `package-architecture.md` 中“future M7 ports待 real consumer closure”的规则完成首次 M7 consumer closure，不建立 universal Platform/Renderer service interface。
...
2. `RendererControlBinding.acquire(token, signal)` 只建立一个 candidate physical carrier；hello 才能授予 currentness。  
3. Main 同时最多一个 current Renderer + 一个 candidate/pending attempt；无 unbounded connection registry。  
4. Initial hello exact outbound preflight发生在 current switch之前。  
5. hello capture/preflight/current-install/old-retire 与 Renderer-visible mutation共享 Main serialization。  
6. Replacement主动撤销 old current，但不假设能取消已开始的 carrier send。  
7. Renderer先安装 initial Snapshot再消费 later state。  
8. Session terminal终止所有 Renderer attempts/current authority，但不等待 Renderer physical close决定 Session result。  
9. Renderer Control representation failure不改变 Frozen Frame / Runtime business authority。  
10. M7 Main DataAuthority保持空；Data policy留给 M8 real consumers。  
11. `OpaqueMaterialGenerator`只供 fresh opaque material，所有语义 authority仍由 Main拥有。  
12. M7不引入 generic RPC、Publisher、Store framework、Renderer Platform mega-port或 shadow authority。  
13. 本 ADR + Frozen Renderer Control contract + `M7_01`–`M7_05` 构成 M7 实施事实源。