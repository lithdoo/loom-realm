...
- 不依赖其他子系统的内部可变状态；
- 不以进程 ID 或 Worker 身份代替 Frame ID。
...
Frame 是调用身份和状态隔离单元，不是进程或 Worker 身份。
...
关闭一个 Frame 不得终止整个 Container，也不得清理其他 Frame 的状态。

## 6. 状态所有权

```text
程序主系统
    System Registry、Runtime Container Registry、Frame、调用关系、Activation、输入目标

模块子系统 Container
    本 System 的共享只读资源和 Frame 实例路由
...
渲染端
    Store、DOM / Canvas / WebGL 和本地表现状态
```
...
Frame 返回只关闭该 Frame Runtime。Container 是否继续常驻由宿主资源策略决定。
...
同一个 Container 中不同 Frame 的 Projector State 和 Revision 必须完全独立。

## 9. 输入与数据通道

每个 Runtime Container 与程序主系统之间有一条长期控制连接。每个 Frame 与 Renderer 之间有一条独立双向数据连接：
...
Renderer ⇄ Frame Runtime
    input.dispatch、node.event、state.snapshot、scope.replace、event.emit、state.resync
```
...
- `frame.activate`：签收 Activation 并开始接收普通输入；
- `frame.suspend`：停止普通输入，可以暂停内部调度；
- `frame.resume`：使用新 Activation 恢复并处理子调用结果；
- `frame.close`：停止新工作并释放该 Frame 的资源。
...
3. Frame 是业务状态和通信隔离单元；
4. 一个 Frame 的关闭和 Resync 不影响其他 Frame；
5. 子系统只能发布自身 Frame 的 Client State；
6. 普通输入与视图状态不经过 Main 转发；
7. 平台承载差异不能改变 System、Frame 和 Activation 语义。
