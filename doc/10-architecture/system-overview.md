...
每个 Frame 独立拥有权威状态、Projector 和数据连接
```
...
负责主系统、Runtime Container、Frame Runtime、渲染端和 Content Service 之间的控制面、Frame 数据面和内容面语义，包括顺序、重连、背压和错误边界。
...
```text
程序主系统
    拥有 System/Container Registry、调用栈、Frame 生命周期、Activation 和 Input Target

Runtime Container
    拥有 System 级共享只读资源和 Frame 实例路由

Frame Runtime
    拥有本次调用的权威业务状态、规则、队列和 Client State Projector

渲染系统
    拥有 Stack/Frame/Scope Store、DOM/Canvas/WebGL 和非权威表现状态
...
PWA 页面进入后台时不保证 Worker 持续运行。宿主必须暂停输入和模拟，并在恢复时重建连接和 Snapshot。
...
同一 System 被多次调用时，可以在同一 Container 内同时存在多个独立 Frame。
...
控制面负责低频严格生命周期；Frame 数据面负责输入、Client State、Event 和 Resync；内容面负责静态数据和资源主体。
...
`loom.map` Container 可以承载多个独立地图 Frame，并共享不可变地图定义和资源索引。
...
3. 每个 Frame 是独立业务状态和数据连接单元；
4. 默认只有活动栈顶接收普通输入；
5. 暂停 Frame 的 Scope 可以继续显示；
6. 跨系统状态通过调用输入、返回结果或正式协议显式传递；
7. 游戏包在运行期间只读；
8. Client State 不暴露物理路径或可执行代码；
9. Renderer Store 是恢复目标，DOM/Scene 不是权威状态；
10. Service Worker 和 Content Service 不拥有游戏运行状态；
11. 桌面与 PWA Transport 不改变协议语义；
12. 第一阶段地图实现不得被推广为所有子系统的公共架构。
