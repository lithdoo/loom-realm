...
> 依赖：[Subsystem Control Protocol v1](./subsystem-control-protocol-v1.md)、[Frame / Call Protocol v1](./frame-call-protocol-v1.md)  
> 最近复核：2026-08-09

本文不新增 Runtime/Frame method。它只定义当前 Main ⇄ Subsystem Control application profile 如何组合 Subsystem Control v1 与已经 Frozen 的 Frame / Call v1。

核心原则：

> **Runtime lifecycle 使用 Subsystem Control v1；Frame transaction 使用 Frame / Call v1。共享同一物理 Control Connection 不意味着共享状态机或错误语义。Data / Render / User Input 不进入本 Profile。**
...
当前 Profile 下 conformant Runtime MUST advertise/support Control version 1；Main 选择 `1` 后，Frame / Call v1 由 Profile 静态绑定。

## 3. Bootstrap / Ready Meaning

Connection 严格按 Subsystem Control v1 bootstrap：
...
表示 Runtime required initialization 已完成，并声明能够完整承担 Frame / Call v1 Subsystem 角色。
...
Frame message继续满足 Frame / Call v1 的 business JsonValue、identity、deadline 与字段 limits；Control message满足 v1 token/version/error limits。

PWA Structured Clone 不得扩大 Frame 或 Control 的 JSON value model。

## 8. Deadlines Remain Domain-specific

共享 Connection 不意味着共享 timeout policy。

Subsystem Control v1 的 connect/hello/ready/shutdown/termination deadline由 Control/Host policy定义。
...
Host/Platform Profile负责安全建立 carrier，并提供一次 Launch Attempt bootstrap credential；建立后必须遵守相同 Control v1 + Frame v1 application semantics。
...
未来如果需要 `Control 2 + Frame 1`、`Control 1 + Frame 2` 或动态 capability/version negotiation，必须发布新的明确 Profile/version；不得静默改变本 Profile。

## 13. Conformance

Main/Subsystem 对 Frame角色的兼容性继续由 [Frame / Call v1 Conformance Profile](./frame-call-conformance-v1.md) 验证。

Profile v1 integration至少覆盖：
...
1. Runtime Control Profile v1 = Subsystem Control v1 + Frame / Call v1；
...
6. `ready` 不携 Data endpoint，并表示完整 Frame v1角色可用；
7. 同一 sender/Connection的 Control + Frame Request共享 one-shot ID namespace；
8. JSON-RPC Batch禁止；
9. 各协议继续拥有自己的 schema/error/deadline semantics；
10. Data/User Input/Render/Content不进入 Runtime Control Profile；
11. Desktop/PWA bootstrap可以不同，但建立后的 application semantics相同。
