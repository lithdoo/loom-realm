...
Web Presentation Config v1 是 M13 current startup contract；ADR 0031 + Rendering System同时冻结 thin Web projector observable semantics，包括 `document.body` root projection、`receiveRenderData(...)`、projection ordering与 no RenderEvent→WC mapping。
...
M13不建立 generic Domain layer/stacking framework；top-level roots直接进入 `document.body`，actual layout/position/stacking由 business WC/CSS负责。
...
Web Projector      mechanical Store → DOM projection
...
Current closed executable root gate是 `npm run test:m12`。M13实现必须消费既有 M11/M12 contracts，并为 WebPresentationConfig/bootstrap + thin projector建立 `npm run test:m13` closure evidence；该 gate必须包含真实 headless Chromium qualification。
...
首次实现前 Current correction按 [文档治理](../00-overview/document-governance.md)传播；ADR 0030是 M12 Content realization closure provenance，ADR 0031与 Web Presentation Config v1定义 M13 current model；均不制造 fake v2/compat layer。
