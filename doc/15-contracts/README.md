...
契约层定义不同系统或不同实现必须共同遵守的可互操作语义。系统架构说明职责和所有权；契约冻结消息、状态、顺序、错误、failure recovery、limits 和兼容性。
...
- recoverable rejection与 control divergence/protocol-fatal分离；
- Runtime failure root=live Stack中 lowest failed-runtime Frame；root..top whole suffix fixed-point unwind；
- failed Runtime Frame可无 close ACK logical retire；healthy descendant best-effort close；
- accepted terminal outcome保留；root无 outcome使用 `SUBSYSTEM_RUNTIME_FAILED`；surviving Caller fresh-resume；
- Frame / Call v1 冻结 message/JSON/identity/failure limits、Request ID profile与 sender-local deadline profile；
- Desktop WebSocket / PWA MessagePort 必须保持同一 Frame / Call application semantics；
- `subsystem.hello.protocolVersions` 仍只协商 Subsystem Control；Frame / Call v1 无独立 handshake/downgrade；
...
绑定 Frame / Call v1 的 deployment/profile 还必须保证每个 `descriptor.key` 可作为 `targetSubsystemKey` 表示（`1..256 UTF-8 bytes`）。这是 Frame profile约束，不改变 Game Package v2 的通用 Descriptor wire。
...
Request ID                 positive safe integer; sender-side Connection lifetime no reuse
Frame deadlines            1,000..300,000 ms sender-local monotonic policy
JSON-RPC Batch             forbidden
```

Desktop one complete WebSocket text message=one JSON-RPC message；PWA one plain JSON-compatible `postMessage` object=one JSON-RPC message；Structured Clone不得扩大 Frame value model。

Conformance companion：[Frame / Call Protocol v1 Conformance Profile](./frame-call-conformance-v1.md)。
...
## 6. Identity / Transaction / Failure / Conformance 分层
...
Runtime Failure Recovery
    failed-runtime set / lowest-root suffix / fixed-point

Frame v1 Completion Profile
    JSON/ID/limits/deadline/transport/version/conformance
...
- 改变 method/field含义、identity ownership、commit point、error classification、unwind root、limits或 ordering guarantee属于不兼容改变；
- Frame / Call v1没有 minor wire version、独立 hello或运行时 downgrade；
- Transport Profile不得改变 Frame application semantics；
