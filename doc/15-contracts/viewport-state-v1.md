...
Raw sample `{width,height}` 均必须 finite positive number，再分别 Math.floor；仅当 floor 后均为 positive safe integer 才接受。`640.9 → 640`；floor 为0、NaN、Infinity、负数、unsafe、missing/extra own property 不成为合法尺寸。对于 trusted raw JS object，source adapter 至少检查普通 object 与 exact own `width`/`height` 两个数字属性，避免读取 prototype/inherited property 或 getter（含 Proxy trap）的不受信任行为；不满足则忽略。非法样本保留上一合法样本，不发送 0/null/default；归一化后同值 suppress。DPR-only 而 CSS logical size 不变不发布。

**Wire 与 raw source 必须区分：** 线上唯一 message 是 JSON 解析得到的 exact **own-key** `{type,width,height}`，width/height 均为 positive safe integer；少字段、多字段、fraction/negative/zero/unsafe value → `viewport` protocol-fatal；仅 Renderer→Subsystem。JSON wire 没有 JS source getter/继承属性的编码，不能构造「继承属性 wire」测试；检查 parsed object 的 exact own-key set 即可。非法 JSON text（例如 `NaN`/`Infinity` 字面量）与通用 byte/depth/representation 错误属于 `profile`，须先于 child validation 拒绝。Trusted local JS outbound caller 提供非法值则走既有 local-fatal send 规则，不归远端 `viewport` 错误。

其他 type/namespace、common 1MiB/depth64、JSON unit、terminal 由 [Profile](./renderer-data-profile-v1.md) 定义；`viewport.foo` → profile unknown。一次消息是完整 retained state，不是 resize action；无 seq、timestamp、ACK、retry token、scale、Map 字段。
