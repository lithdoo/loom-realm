...
| 系统 | 模块入口 | 说明 |
|---|---|---|
| 程序主系统 | [main-system](./main-system/README.md) | Descriptor Registry、Launcher、Runtime Registry、Frame Stack、Control 与 Data Grant |
| Web 渲染端 | [web-renderer](./web-renderer/README.md) | Main Control、System Connection、Render Store、Frame Input、DOM/Canvas/WebGL |
| 游戏包与内容 | [game-package](./game-package/README.md) | Manifest/Entry/Descriptor Loader、Catalog、Repository、Validator |
...
| 桌面宿主 | [desktop-host](./desktop-host/README.md) | Hostra Window Adapter、Desktop WebSocket/HTTP 与 Process Supervisor |
| PWA 宿主 | [pwa-host](./pwa-host/README.md) | Main/System Worker、每 Subsystem Data MessagePort、Service Worker 和 OPFS |
...
进程 / Worker 隔离粒度是 Subsystem，不是 Frame。
...
仍存在的旧目录资料只作为迁移/历史参考。与当前模块设计冲突的旧结论必须逐步降级为 Legacy，而不是继续作为实现入口。