...
> **`loom.map` 只实现地图业务；Game Entry只声明 logical key，Hostra/PWA launchers各自选择满足同一 SubsystemDefinitionFactory ABI 的 build artifact；业务 source不包含平台/协议机械分支。**
...
哪个 `.mjs` artifact实现 `loom.map` 由 current Platform Launch Manifest/Plan声明：

```text
launch.hostra.json → Hostra artifact
launch.pwa.json    → PWA artifact
```

两者可以指向同一 portable build，也可以指向不同 build artifact；default export ABI和 observable business semantics必须相同。

Definition Module加载本身不启动 Runtime，也不读取 Platform bootstrap/launch manifest。
...
Executable module resolution也不通过 ContentClient暴露给地图业务。
...
@loomrealm/game-package runtime config APIs
...
Build / installation boundary可以产生：

```text
map source
   ├→ Hostra build artifact
   └→ PWA build artifact
```

或直接复用一个 portable artifact。

关键要求不是“同一个文件”，而是：

```text
same logical key = loom.map
same SubsystemDefinitionFactory ABI
same business behavior for same logical input
```

业务不得读取 `launch.hostra.json` / `launch.pwa.json` 或通过 `if (platform)`改变业务语义。
...
platform-selected-artifact-same-author-ABI
frame-params
...
Hostra-artifact-vs-PWA-artifact-same-abstract-business-trace
business-cannot-read-platform-launch-config
```
...
map reads Game/Platform launcher config
map chooses its own Process/Worker/runtime module
...
platform-specific business semantics branch
```
...
> **`loom.map` 证明一个真正普通的业务 Subsystem：只依赖统一 author SDK；Game Package只认识 logical `loom.map` key；不同 Platform可以用自己的 launch binding选择不同或相同 Definition artifact，但 Frame authority、Data reconnect、Platform provisioning、wire mechanics与 executable realization都留在正确下层，同时得到同一业务语义。**
