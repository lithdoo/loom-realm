...
class SubsystemHost {
  ...
  private readonly viewportView: Viewport = (() => {
    const manager = this.viewport;
    return Object.freeze({
      get current() { return manager.current; },
      subscribe: (listener: (value: ViewportSize | null) => void) => manager.subscribe(listener),
    });
  })();
      ...
      peer = createSubsystemDataPeer({
        ...
        handlers: {
          ...
          onViewportState: (message) => {
            if (this.currentDataPeer === peer) this.viewport.onViewportState(message);
