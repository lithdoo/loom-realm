...
/**
 * Exact protocol family for an already-recognized message type. Unknown
 * `viewport.*` names remain `profile` because only `viewport.state` is
 * recognized; this helper never widens the recognized surface.
 */
export function protocolFamilyForType(type: string): DataProtocolFamily {
...
function validateByType(raw: unknown): RendererDataMessageV1 {
  ...
  fail(protocolFamilyForType(type), "unknown data message type");
}
...
export function decodeForRole(raw: string, role: DataRole): RendererDataMessageV1 {
  ...
  if (!inboundAllowed(role, t)) {
    fail(protocolFamilyForType(t), "message direction invalid for role");
  }
  return message;
}
...
export function encodeForRole(message: RendererDataMessageV1, role: DataRole): string {
  ...
  if (!outboundAllowed(role, t)) {
    fail(protocolFamilyForType(t), "outbound direction invalid for role");
  }
