...
test("V-04/P3-02: recognized viewport schema/direction → viewport; invalid JSON/unknown → profile; local invalid → local-fatal", async () => {
  {
...
test("V-05: A→B→A while inFlight cancels B and does not resend A", async () => {
  const sent = [];
  let release;
...
test("P3-01: viewport.state only Renderer→Subsystem; reverse is viewport fatal", async () => {
  const pair = createMemoryCarrierPair();
  const renderer = createRendererDataPeer({
    binding: binding(pair.right),
    handlers: rendererHandlers,
