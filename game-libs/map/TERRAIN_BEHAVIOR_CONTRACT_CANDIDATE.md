...
| CI | `.github/workflows/essentials-fixture.yml` → `npm run test:fixtures`。失败 run [35499223917](https://github.com/lithdoo/loom-realm/actions/runs/35499223917) on `faf4c64`。绿 run [35499617524](https://github.com/lithdoo/loom-realm/actions/runs/35499617524) on `5b550b4`：**100 pass / 0 fail / 6 skip**。live skip ≠ PASS。无许可则 FG-05 不得 PASS |
...
| DEC-06 fixture/CI | 合成世界可提交；真图 live 留本机。附录 A 不扩大 | `npm run test:fixtures`；绿 run `35499617524` = 100/0/6 skip | FG-05 **OPEN** 直至绿 CI **且** 许可签核；live skip ≠ PASS |
...
3. **CI workflow run：** 绿 run `35499617524` on `5b550b4` 已记录（100 pass / 6 skip）。绿合成 ≠ FG-05 PASS。许可仍缺。
