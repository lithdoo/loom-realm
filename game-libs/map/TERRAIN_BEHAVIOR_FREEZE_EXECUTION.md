...
> **PREPARATION IN PROGRESS / NOT FROZEN / NOT IMPLEMENTED / NOT QUALIFIED**。这是本轮直接执行的冻结准备工件，不是对既有 6 个 gate 的代签。起点：分支 `docs/map-terrain-behavior-freeze-handoff`，审核基线 `b26a4d68d1cbe4136242e0b4fd09dd2d617e56a6`，取证提交 `e60e4a521a726233bbda0bb1892f6d25bc47573d`。创建本文件及合同候选/任务卡是准备交付，不能计作原版动态验证、CI PASS、实现或正式冻结。
>
...
- 本次没有本地官方 FSDB 或可执行 RGSS 运行时，因此不能新产生 `.local/` 完整路线输出、逐帧日志、合法 golden fixture、许可结论或 CI run/SHA。缺失的证据记 `BLOCKED` 或 `OPEN`，不记 PASS。

## 2. 强制交付顺序与完成判据

| 顺序 / 卡 | 要交付的实际工件 | 硬性完成判据 | 当前 |
|---|---|---|---|
| **FZ-00 锁定事实** | 固定上游 SHA/三图文件 digest/本地分支及 CI run；核对源/目标 mapId 与原始事件命令 | 审查者能重放源文件 SHA 与源代码路径，记录环境、命令、退出码、skip；不改历史证据 | 旧证据有；本轮独立重放 BLOCKED（无 FSDB） |
| **FZ-01 E2E-21 静态拼接** | 单一状态 machine 的 Map7 edge→Map21 landing→BFS→Off→On→**真实 Bridge tile**→Off→折返/回 Map7 的统一 trace 与负例 | 每步连续 mapId/x/y/direction/bridgeLevel、事件 ID/page/start/execute 分离、transfer、pre/post 与输入可用时刻；与原始图层/连接独立核对；有针对性回归且真实 FSDB 运行 | OPEN，见 §3 |
| **FZ-02 原版动态** | v21.1 RGSS 桥、跳跃、事件重复、held input、地图传送、相机/帧日志 | 每组有输入/初态/帧次/事件 start 与 execute/桥层/坐标/截图或结构化日志，和静态输出逐项对照 | BLOCKED：本执行环境无原版 RGSS；门禁不能自改为可选 |
| **FZ-03 Map47 行为边界** | 两格 jump 的方向、源格、mid、land、事件/阻挡、边界/跨图支持矩阵 | 真实 Map47 与明确标记的合成负例分开；动态必需范围同 FZ-02；不以 `jump=2 walks` 替代 | 静态样本有；完整状态与动态 OPEN/BLOCKED |
| **FZ-04 许可+fixture+CI** | 可合法分发的最小 fixture、派生过程和 digest；可在 CI 实跑的 map test workflow | 可追溯许可；CI run/commit SHA + pass/fail/skip 逐项；真实 live 项不能 skip 当 PASS；处理既有 Map7 附录 A 许可风险 | OPEN：暂无已证实合法 fixture、CI 实测 |
| **FZ-05 C-01～08 决策** | [合同候选](./TERRAIN_BEHAVIOR_CONTRACT_CANDIDATE.md) 的 DEC-01～07 全部证据化裁决，单一规范、版本、exact TS/JSON、反例矩阵 | Producer/consumer、MapAction、MapTransfer、bridgeLevel、事件时间、Motion/Browser payload、数据迁移、资格测试全部无 OPEN 或相互矛盾条款 | 候选已写，决策 OPEN |
| **FZ-06 资格与签核** | [门禁](./TERRAIN_BEHAVIOR_FREEZE_READINESS.md) 逐项证据/审核人/日期，正式任务卡 baseline、M14/M15 subject/CI 精确关联 | 六个 gate 经授权审查者独立 PASS，所有要求的来源和 workflow SHA 可访问，才创建/签核唯一 `CONTRACT_V1` 并声明 `Contract Frozen / Implementation Pending` | OPEN；不能由本文自动 PASS |
...
|---|---|---|---|
| FG-01 原版事实 | 7/21/47 静态源数据、触发与跳跃样本 | FZ-01 单 trace，FZ-02 原版事件/动作时序，FZ-03 支持界限和审查 | OPEN |
| FG-02 数据导入 | 真实 Map21 PBS 3 行/7 edge，源/目标空间分离，D0 风险审计 | C-01/03 schema/Content/动态事实保留/迁移签核；不得杜撰已有误删 | OPEN |
| FG-03 事件状态 | 原版源码 start≠execute，8 事件静态 here | held input/重复 start 与 execute、bridgeLevel、转图时点与原子状态矩阵 | OPEN |
| FG-04 运动画面 | 现有 250ms walk、scene/visual epoch、RenderDomain 事实 | jump exact ABI/帧、resize/取消/transfer、原地桥 depth 缓存 invalidation | OPEN |
| FG-05 复现资格 | 本地 Agent 旧 36/36 与输入指纹 | 合法 fixture、CI 对应 run/SHA、完整 golden expected、许可确认 | OPEN |
| FG-06 Agent 交接 | 设计、分期和**候选**合同/任务卡 | C-01～08 唯一合同签核，任务卡 exact implementation base，M14/M15 qualification reviewer | OPEN |
...
**本轮可执行准备的出口**：候选合同、逐项可验证任务、Git 基线和签核表已具备；没有实测证据的缺口仍明确 OPEN/BLOCKED，不能冒充“冻结任务已经完成”或授权 AG-01～04。